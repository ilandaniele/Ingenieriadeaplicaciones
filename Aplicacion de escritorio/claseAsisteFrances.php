...
class Asiste
{
    ...
    public static function getAsistes() 
		{
			...
			$obj_asiste->executeQuery("select * from ASISTE_FRANCES"); 
	...
	{	
		...
		{
			$obj_asiste=new sQuery();
			$result=$obj_asiste->executeQuery("select * from ASISTE_FRANCES where dni = '$nro' and id ='$nro2'"); // ejecuta la consulta para traer al evento
	...
	{		
				$obj_asiste=new sQuery();
				$query="update ASISTE_FRANCES set dni = '$this->dniAux', id='$this->idAux' where dni = '$this->dni' and id ='$this->id'";
	...
	{		
			...
			$obj_asiste=new sQuery();
			$query="insert into ASISTE_FRANCES(dni,id) values ('$this->dniAux','$this->idAux')"; 
	...
	{
			$obj_asiste=new sQuery();
			$query="delete from ASISTE_FRANCES where dni='$this->dni' and id='$this->id'";
