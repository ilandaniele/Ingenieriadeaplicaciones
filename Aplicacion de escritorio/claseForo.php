...
class Foro
{
    ...
    public static function getForos() 
		{
			$obj_foro=new sQuery();
			$obj_foro->executeQuery("select * from FORO"); 
	...
	function Foro($nro=0) 
	{
		...
		{
			$obj_foro=new sQuery();
			$result=$obj_foro->executeQuery("select * from FORO where codigo = '$nro'"); 
	...
	{	
				$obj_foro=new sQuery();
				$query="update FORO set codigo = '$this->codigoAux', nombre='$this->nombre',  detalle='$this->detalle', 
	...
	{		
			...
			$obj_foro=new sQuery();
			$query="insert into FORO(codigo, nombre, detalle, cod_postal) values
	...
	{
			$obj_foro=new sQuery();
			$query="delete from FORO where codigo='$this->codigo'";
