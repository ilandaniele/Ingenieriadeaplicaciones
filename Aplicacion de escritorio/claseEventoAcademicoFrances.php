...
class Evento
{
    ...
    public static function getEventos() 
		{
			...
			$obj_evento->executeQuery("select * from EVENTOACADEMICO_FRANCES"); // ejecuta la consulta para traer a los eventos
	...
	//hay que cambiarle el atributo clave al evento, y en vez de ponerle el nombre, ponerle el id, y vuelvo a poner el atributo hidden en ventanaEvento, que es el id
		...
		{
			$obj_evento=new sQuery();
			$result=$obj_evento->executeQuery("select * from EVENTOACADEMICO_FRANCES where id = '$nro'"); // ejecuta la consulta para traer al evento
	...
	{		
				$obj_evento=new sQuery();
				$query="update EVENTOACADEMICO_FRANCES set id = '$this->idAux', nombre='$this->nombre', aula='$this->aula' where id = '$this->id'";
	...
	{		
			...
			$obj_evento=new sQuery();
			$query="insert into EVENTOACADEMICO_FRANCES(id, nombre, aula)values
	...
	{
			$obj_evento=new sQuery();
			$query="delete from EVENTOACADEMICO_FRANCES where id='$this->id'";
