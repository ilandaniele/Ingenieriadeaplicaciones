...
class CiudadIngles
{
    ...
    public static function getCiudad() 
		{
			...
			$obj_ciudad->executeQuery("select * from CIUDAD_INGLES");

			return $obj_ciudad->fetchAll(); // retorna todas las ciudades
		}

	function CiudadIngles($cod_postal=0) // declara el constructor
	{	
		if ($cod_postal!=0)
		{
			$obj_ciudad=new sQuery();
			$result=$obj_ciudad->executeQuery("select * from CIUDAD_INGLES where cod_postal = '$cod_postal'"); 
	...
	{		
				$obj_ciudad=new sQuery();
				$query="update CIUDAD_INGLES set cod_postal = '$this->cod_postalAux', nombre='$this->nombre', inf_turistica='$this->inf_turistica'";
	...
	{		
			...
			$obj_ciudad=new sQuery();
			$query="insert into CIUDAD_INGLES(cod_postal, nombre, inf_turistica)values
	...
	{
			$obj_ciudad=new sQuery();
			$query="delete from CIUDAD_INGLES where cod_postal='$this->cod_postal'";
