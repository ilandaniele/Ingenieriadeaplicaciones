...
class Evento
{
    ...
    public static function getEventos() 
		{
			...
			$obj_evento->executeQuery("select * from EVENTO_FRANCES"); // ejecuta la consulta para traer a los eventos
	...
	//hay que cambiarle el atributo clave al evento, y en vez de ponerle el nombre, ponerle el id, y vuelvo a poner el atributo hidden en ventanaEvento, que es el id
		...
		{
			$obj_evento=new sQuery();
			$result=$obj_evento->executeQuery("select * from EVENTO_FRANCES where id = '$nro'"); // ejecuta la consulta para traer al evento
	...
	{		
				$obj_evento=new sQuery();
				$query="update EVENTO_FRANCES set id = '$this->idAux', nombre='$this->nombre', lugar='$this->lugar',
	...
	{		
			...
			$obj_evento=new sQuery();
			$query="insert into EVENTO_FRANCES(id, nombre, lugar, fecha, horainicio, horafin, detalle, nombre_foro)values
	...
	{
			$obj_evento=new sQuery();
			$query="delete from EVENTO_FRANCES where id='$this->id'";
