...
class Foro
{
    ...
    public static function getForos() 
		{
			$obj_foro=new sQuery();
			$obj_foro->executeQuery("select * from FORO_FRANCES"); 
	...
	function Foro($nro=0) 
	{
		...
		{
			$obj_foro=new sQuery();
			$result=$obj_foro->executeQuery("select * from FORO_FRANCES where codigo = '$nro'"); 
	...
	{	
				$obj_foro=new sQuery();
				$query="update FORO_FRANCES set codigo = '$this->codigoAux', nombre='$this->nombre',  detalle='$this->detalle', 
	...
	{		
			...
			$obj_foro=new sQuery();
			$query="insert into FORO_FRANCES(codigo, nombre, detalle, cod_postal) values
	...
	{
			$obj_foro=new sQuery();
			$query="delete from FORO_FRANCES where codigo='$this->codigo'";
