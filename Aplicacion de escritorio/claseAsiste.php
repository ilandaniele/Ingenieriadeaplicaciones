...
class Asiste
{
    ...
    public static function getAsistes() 
		{
			...
			$obj_asiste->executeQuery("select * from ASISTE"); 
	...
	{	
		...
		{
			$obj_asiste=new sQuery();
			$result=$obj_asiste->executeQuery("select * from ASISTE where dni = '$nro' and id ='$nro2'"); // ejecuta la consulta para traer al evento
	...
	{		
				$obj_asiste=new sQuery();
				$query="update ASISTE set dni = '$this->dniAux', id='$this->idAux' where dni = '$this->dni' and id ='$this->id'";
	...
	{		
			...
			$obj_asiste=new sQuery();
			$query="insert into ASISTE(dni,id) values ('$this->dniAux','$this->idAux')"; 
	...
	{
			$obj_asiste=new sQuery();
			$query="delete from ASISTE where dni='$this->dni' and id='$this->id'";
