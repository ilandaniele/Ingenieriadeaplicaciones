...
class Evento
{
    ...
    public static function getEventos() 
		{
			...
			$obj_evento->executeQuery("select * from EVENTOSOCIAL_FRANCES"); // ejecuta la consulta para traer a los eventos
	...
	//hay que cambiarle el atributo clave al evento, y en vez de ponerle el nombre, ponerle el id, y vuelvo a poner el atributo hidden en ventanaEvento, que es el id
		...
		{
			$obj_evento=new sQuery();
			$result=$obj_evento->executeQuery("select * from EVENTOSOCIAL_FRANCES where id = '$nro'"); // ejecuta la consulta para traer al evento
	...
	{		
				$obj_evento=new sQuery();
				$query="update EVENTOSOCIAL_FRANCES set id = '$this->idAux', nombre='$this->nombre', interes='$this->interes' where id = '$this->id'";
	...
	{		
			...
			$obj_evento=new sQuery();
			$query="insert into EVENTOSOCIAL_FRANCES(id, nombre, interes)values
			('$this->idAux','$this->nombre', '$this->interes') 
			ON DUPLICATE KEY UPDATE id = '$this->id', nombre='$this->nombre', interes='$this->interes'";
	...
	{
			$obj_evento=new sQuery();
			$query="delete from EVENTOSOCIAL_FRANCES where id='$this->id'";
