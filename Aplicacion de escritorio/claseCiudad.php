...
class Ciudad
{
    ...
    public static function getCiudad() 
		{
			...
			$obj_ciudad->executeQuery("select * from CIUDAD");

			return $obj_ciudad->fetchAll(); // retorna todas las ciudades
		}

	function Ciudad($cod_postal=0) // declara el constructor
	{	
		if ($cod_postal!=0)
		{
			$obj_ciudad=new sQuery();
			$result=$obj_ciudad->executeQuery("select * from CIUDAD where cod_postal = '$cod_postal'"); 
	...
	{		
				$obj_ciudad=new sQuery();
				$query="update CIUDAD set cod_postal = '$this->cod_postalAux', nombre='$this->nombre', inf_turistica='$this->inf_turistica'";
	...
	{		
			...
			$obj_ciudad=new sQuery();
			$query="insert into CIUDAD(cod_postal, nombre, inf_turistica)values
	...
	{
			$obj_ciudad=new sQuery();
			$query="delete from CIUDAD where cod_postal='$this->cod_postal'";
